...
const StatsOverview = ({ stats, setActiveTab }) => {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-fadeIn">
      ...
      {/* === LEFT COLUMN: Action Items Stack === */}
      <div className="flex flex-col gap-6">
        
        {/* Pending Managers Card */}
        <div
      ...
      {/* We use sm:grid-cols-2 to ensure it splits into 2 cols on tablet/desktop */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        
        {/* Row 1: Active Managers (Left) */}
        <StatCard
        ...
        {/* Row 2: Total Routes (Centered & Same Size) */}
        {/* Explanation of classes:
            - sm:col-span-2: Spans the full width of the container row
            - sm:justify-self-center: Centers the card in that row
            - sm:w-[calc(50%-0.75rem)]: Forces width to match a single column card 
              (50% width minus half of the 1.5rem gap)
        */}
        <StatCard
...
const StatCard = ({ label, value, icon, className = "" }) => (
  <div className={`bg-white border border-slate-100 rounded-3xl p-8 shadow-sm flex flex-col justify-between h-full group hover:shadow-md transition-all ${className}`}>
