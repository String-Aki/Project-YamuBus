...
import { FaBus, FaSave, FaLink, FaCheckCircle, FaLock, FaEnvelope, FaServer } from 'react-icons/fa'; 
...
const SetupBus = () => {
  ...
  const handleLoadFleet = async () => {
    ...
    try {
        ...
        const { data } = await axios.post(`${API_URL}/fleetmanagers/setup/buses`, {
            idToken: idToken
        });
        ...
        console.error(error);
        const errMsg = error.code ? error.code.replace('auth/', '') : "Authentication Failed";
        alert(errMsg);
    } finally {
        setLoading(false);
    }
  ...
  return (
    <div className="min-h-screen bg-[#0f1115] text-white p-6 flex flex-col items-center justify-center font-sans">
        ...
        <div className="absolute top-6 left-6 flex items-center gap-2 opacity-50">
            <FaServer /> <span className="text-xs font-mono tracking-widest uppercase">System Config v1.0</span>
        </div>
        ...
        <div className="w-full max-w-md bg-[#1a1d21] p-8 rounded-2xl border border-gray-800 shadow-2xl relative overflow-hidden">
            ...
            <div className="text-center mb-10 mt-2">
                <div className="w-20 h-20 bg-[#252830] rounded-full flex items-center justify-center mx-auto mb-5 text-yellow-500 shadow-inner border border-gray-700">
                    ...
                    {step === 1 ? <FaLock className="text-3xl" /> : <FaLink className="text-3xl" />}
                </div>
                
                <h1 className="text-2xl font-bold tracking-tight">Device Kiosk Setup</h1>
            ...
            {step === 1 ? (
                <div className="space-y-5">
                    <div>
                        <label className="text-[10px] font-bold text-gray-500 uppercase tracking-wider ml-1">Manager Credentials</label>
                ...
                <div className="space-y-4">
                    <div className="bg-[#252830] p-3 rounded-lg border border-gray-700 text-center">
                        ...
                        <p className="text-white font-mono text-sm truncate px-4">{email}</p>
                    </div>

                    <div className="text-xs font-bold text-gray-500 uppercase ml-1 mt-4">Available Vehicles</div>
                        ...
                        {buses.map((bus) => (
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    <FaBus />
                                    <span className="font-mono text-sm font-bold tracking-wide">{bus.licensePlate}</span>
                                </div>
        ...
        <div className="mt-8 text-center opacity-30 text-[10px] max-w-xs leading-relaxed">
            <p>WARNING: This action permanently associates this hardware ID with the selected vehicle database record.</p>
        </div>
    </div>
  );
};
