...
import { FaPowerOff, FaBus, FaMapMarkerAlt, FaWifi, FaExclamationTriangle } from 'react-icons/fa';
...
const Dashboard = () => {
  ...
  const handleStartTrip = async () => {
    ...
    try {
        // 2. Call Backend
        const { data } = await axios.post('http://192.168.43.185:5000/api/trips/start', 
            { busId }, 
            { headers: { Authorization: `Bearer ${token}` } }
        );

        // 3. Save Trip ID (Vital for ending it later)
        ...
        alert(msg); // Alert is fine for now, we can make it pretty later
    }
  };
    ...
    <div className="min-h-screen bg-[#121418] text-white flex flex-col font-sans">
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
        ...
        <div className="mb-10 animate-fade-in-up">
            <h2 className="text-gray-500 text-sm font-bold uppercase tracking-[0.2em] mb-2">Vehicle ID</h2>
        ...
        >
            ...
            <span className="text-2xl font-black tracking-widest text-white drop-shadow-md">START TRIP</span>
            <span className="text-xs text-green-200 mt-1 font-medium">TAP TO BEGIN ROUTE</span>
      ...
      <div className="p-6">
        <button 
            ...
            className="w-full bg-[#1a1d21] border border-gray-700 text-gray-400 py-4 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-red-500/10 hover:text-red-500 hover:border-red-500/50 transition-all"
        >
