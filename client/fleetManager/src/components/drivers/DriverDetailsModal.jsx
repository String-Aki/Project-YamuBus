...
const DriverDetailsModal = ({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await axios.put(
        `${API_URL}/fleetmanagers/drivers/${driver._id}`,
        formData,
        config,
      );
  ...
  const handleDelete = async () => {
    if (
      !window.confirm(
        "Are you sure? This driver will be permanently removed and cannot log in anymore.",
      )
    )
      return;
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h2 className="text-2xl font-bold text-gray-800 leading-none">
              {isEditing ? "Edit Driver" : driver.name}
            </h2>
            {!isEditing && (
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-3 mt-6">
              <button
                ...
                className="flex-1 bg-brand-brown text-white py-3 rounded-xl font-bold flex items-center justify-center gap-2 shadow-lg active:scale-95 transition-transform"
              >
                <FaEdit /> Edit Details
              </button>
              <button
                onClick={handleDelete}
                className="w-14 bg-red-50 text-red-500 rounded-xl flex items-center justify-center hover:bg-red-100 active:scale-95 transition-transform"
              >
                <FaTrash />
              </button>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-3 mt-6">
              <button
                onClick={handleSave}
                disabled={loading}
                className="flex-1 bg-green-600 text-white py-3 rounded-xl font-bold flex items-center justify-center gap-2 shadow-lg active:scale-95 transition-transform"
              >
                {loading ? (
                  "Saving..."
                ) : (
