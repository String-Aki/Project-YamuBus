...
import { FaArrowLeft, FaEye, FaEyeSlash, FaCamera, FaFileAlt, FaBuilding, FaChevronDown } from 'react-icons/fa';
...
const CreateAccount = () => {
  ...
  const handleNext = async () => {
    if (currentStep === 1) {
      ...
      if (!formData.organizationName || !formData.contactPhone || !formData.nicID) return alert("Please complete all details");
      setCurrentStep(3);
    } 
    else if (currentStep === 3) {
      if (!nicFront || !nicBack || !verificationDoc) return alert("All verification documents are required.");
      handleSubmit();
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const docRef = ref(storage, `verification_${firebaseUser.uid}_doc.pdf`);

      const [frontSnap, backSnap, docSnap] = await Promise.all([
        uploadBytes(frontRef, nicFront),
        uploadBytes(backRef, nicBack),
        uploadBytes(docRef, verificationDoc) 
      ]);

      const frontURL = await getDownloadURL(frontSnap.ref);
      const backURL = await getDownloadURL(backSnap.ref);
      const docURL = await getDownloadURL(docSnap.ref);
      ...
      alert("Account Application Submitted! Admin will review your documents.");
      navigate('/login');
  ...
  const namePlaceholder = isPrivate ? "Company Name (e.g. Kandy Super Line)" : "Depot Name (e.g. Kandy South)";
  const docTitle = isPrivate ? "Business Registration / Permit" : "Appointment Letter / ID";
  const docBg = isPrivate ? "bg-yellow-50 border-yellow-400" : "bg-blue-50 border-blue-400";
    ...
    <div className="flex flex-col h-[100dvh] bg-brand-brown overflow-hidden font-sans">
      ...
      <div className="flex-none p-6 pt-8 text-white min-h-[120px]">
        ...
        <div className="text-right">
          <h1 className="text-4xl font-semibold leading-tight drop-shadow-md">
            {currentStep === 1 ? "Create\nAccount" : currentStep === 2 ? "Operator\nDetails" : "Verify\nIdentity"}
          </h1>
        </div>
      </div>
      ...
      <div className="flex-1 bg-brand-dark rounded-tl-[3rem] px-6 py-6 flex flex-col items-center w-full shadow-2xl relative">
        ...
        <div className="flex gap-4 mb-6 flex-none">
          {[1, 2, 3].map((step) => (
            <div key={step} className={`transition-all duration-300 ${currentStep === step ? 'w-12 bg-brand-brown' : 'w-3 bg-gray-600'} h-3 rounded-full shadow-sm`}></div>
          ))}
        </div>
        ...
        <div className="w-full flex-1 overflow-y-auto custom-scrollbar pr-1 pb-4">
          ...
          {currentStep === 3 && (
            <div className="flex flex-col gap-4 w-full animate-fadeIn">
              ...
              <input type="file" ref={docInputRef} onChange={(e) => handleImageChange(e, 'doc')} className="hidden" accept="image/*,application/pdf" />

              <div className="grid grid-cols-2 gap-3">
                 <div onClick={() => frontInputRef.current.click()} className="h-32 bg-white rounded-2xl flex flex-col items-center justify-center cursor-pointer overflow-hidden shadow-md relative">
                    {previewFront ? <img src={previewFront} alt="Front" className="w-full h-full object-cover" /> : <><FaCamera className="text-2xl text-gray-400 mb-2" /><span className="text-gray-500 font-bold text-xs">NIC Front</span></>}
                 </div>
                 <div onClick={() => backInputRef.current.click()} className="h-32 bg-white rounded-2xl flex flex-col items-center justify-center cursor-pointer overflow-hidden shadow-md relative">
                    {previewBack ? <img src={previewBack} alt="Back" className="w-full h-full object-cover" /> : <><FaCamera className="text-2xl text-gray-400 mb-2" /><span className="text-gray-500 font-bold text-xs">NIC Back</span></>}
                 </div>
              </div>
                ...
                {previewDoc ? (
                    verificationDoc?.type === 'application/pdf' ? (
                        <div className="flex flex-col items-center justify-center h-full w-full bg-gray-50 p-2">
                            ...
                            <span className="text-xs font-bold text-gray-700 text-center truncate w-full">{verificationDoc.name}</span>
                            <span className="text-[10px] text-red-500 font-bold mt-1">PDF Selected</span>
                        </div>
                    ) : (
                        <img src={previewDoc} alt="Doc" className="w-full h-full object-cover" />
                    )
                ) : (
                    <>
                        <FaFileAlt className={`text-3xl mb-2 ${docColor}`} />
                        <span className={`font-bold text-sm ${docColor}`}>{docTitle}</span>
                        <span className="text-gray-500 text-[10px] uppercase tracking-wider mt-1">Required Verification</span>
                    </>
                )}
              </div>
            </div>
        ...
        <div className="w-full mt-auto pt-4 pb-2">
          ...
          {currentStep === 1 && (
             <p className="text-white text-sm mt-4 text-center opacity-80">
                Already have an account? <span className="font-bold underline cursor-pointer hover:text-brand-brown" onClick={() => navigate('/login')}>Sign In</span>
             </p>
          )}
        </div>
